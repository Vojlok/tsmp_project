...
{
	if(lanim&&xr_strlen(lanim)) m_lanim	= LALib.FindItem(lanim);
	else m_lanim	= NULL;
}

void CUICustomEdit::SendMessage(CUIWindow* pWnd, s16 msg, void* pData)
{		
		if(msg == WINDOW_KEYBOARD_CAPTURE_LOST) //кто-то другой захватил клавиатуру
		{
			m_bInputFocus = false;
			m_iKeyPressAndHold = 0;
		}
...
bool CUICustomEdit::KeyPressed(int dik)
{
	...
	if (m_bNumbersOnly)
	{
		if (strstr(m_lines.GetText(), ".")) return true;
		...
		if(bChanged) GetMessageTarget()->SendMessage(this,EDIT_TEXT_CHANGED,NULL);
...
void CUICustomEdit::AddChar(char c)
{
	...
	{
		if (m_lines.GetVisibleHeight() > GetHeight()) m_lines.DelLeftChar();
	}
}
...
void CUICustomEdit::Enable(bool status)
{
	...
	if (!status) SendMessage(this,WINDOW_KEYBOARD_CAPTURE_LOST);
}
...
void CUICustomEdit::SetText(LPCSTR str) { CUILinesOwner::SetText(str); }
