...
void CLocatorAPI::Register		(LPCSTR name, u32 vfs, u32 crc, u32 ptr, u32 size_real, u32 size_compressed, u32 modif)
{
	...
	else desc.name = xr_strdup(desc.name);	
	...
	while (temp[0]) 
	{
		...
		if (!exist(path))	
		{
            ...
            desc.modif			= u32(-1);
            std::pair<files_it,bool> II = files.insert(desc); 
...
void CLocatorAPI::_initialize(u32 flags, LPCSTR target_folder, LPCSTR fs_name)
{
	...
	{ //find nearest fs.ltx and set fs_root correctly
		...
		if (!pFSltx)
		{
			...
			else append_path ("$fs_root$", "", 0, FALSE);
...
T *CLocatorAPI::r_open_impl	(LPCSTR path, LPCSTR _fname)
{
	...
	if (!check_for_file(path,_fname,fname,desc)) return	(0);
...
{
	if(m_Flags.test(flDumpFileActivity)) _unregister_open_file(fs);
...
void CLocatorAPI::unlock_rescan()
{
	...
	if ((0==m_iLockRescan)&&m_Flags.is(flNeedRescan)) rescan_pathes();
}
