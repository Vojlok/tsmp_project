...
//				clMsg			("processing: %s",th_name);			// commented by KD
	...
	if (is_thm_missing) 	MessageBox(Window, "Some of required thm's are missing. See log for details.", "Error!", MB_OK | MB_ICONERROR);
	if (is_tga_missing) 	MessageBox(Window, "Some of required tga textures are missing. See log for details.", "Error!", MB_OK | MB_ICONERROR);
