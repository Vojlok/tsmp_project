...
void	CTextConsole::CreateConsoleWnd()
{
	...
	WNDCLASS wndClass = { 0, TextConsole_WndProc, 0, 0, hInstance, 
		...
		NULL, wndclass }; 
	RegisterClass( &wndClass ); // Register the windows class
	
	u32 dwWindowStyle = WS_OVERLAPPED | WS_CHILD | WS_VISIBLE; // Set the window's initial style
		
	RECT rc;
	SetRect			( &rc, lX, lY, lWidth, lHeight ); // Set the window's initial width
...
void	CTextConsole::CreateLogWnd()
{
	...
	m_hLogWnd = CreateWindow(wndclass, "XRAY Text Console Log", dwWindowStyle,
		lX, lY, lWidth, lHeight, m_hConsoleWnd, 0, hInstance, 0L );
